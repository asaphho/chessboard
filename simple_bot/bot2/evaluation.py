...
# ACTIVITY MULTIPLIER
ACTIVITY_COUNT_MULTIPLIER = 0.07  # BASE
CENTRAL_SQUARE_BONUS = 0.03
SQUARE_AROUND_ENEMY_KING = 0.03

# MATERIAL THREAT MULTIPLIER
MATERIAL_THREAT_MULTIPLIER = 0.07
CHECKMATE_THREAT_SCORE = 5.5
...
def new_evaluate(position: Position) -> Dict[str, float]:
    ...
    square_piece_dict = position.white_pieces.get_square_piece_symbol_dict() | position.black_pieces.get_square_piece_symbol_dict(
        lowercase=True)
        ...
        if piece in ('p', 'P'):
            if piece == 'p':
                ...
                for controlled_square in squares_controlled:
                    score += BLACK_PAWN_CONTROL_SCORES[controlled_square] if side_evaluating_for == 'b' else - \
                    BLACK_PAWN_CONTROL_SCORES[controlled_square]
            else:
                squares_controlled = position.scan_pawn_attacked_squares('w', sq)
                for controlled_square in squares_controlled:
                    score += WHITE_PAWN_CONTROL_SCORES[controlled_square] if side_evaluating_for == 'w' else - \
                    WHITE_PAWN_CONTROL_SCORES[controlled_square]
