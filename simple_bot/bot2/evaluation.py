...
from utils.board_functions import (
    scan_kn_scope, INT_SQUARES_MAP, LINE_EXTEND_MAP, PIECE_MOVE_TYPE_DICT,
    SQUARE_SCOPES_MAP, LETTER_TO_NUM, NUM_TO_LETTER
)
...
def is_pinned(king_sq: str, king_color: str, pns: str, target_sq: str,
              square_piece_dict: Dict[str, str], ignore_target_sq: bool = False) -> Union[str, None]:
...
def is_doubled_pawn(square: str, color: str, square_piece_dict: Dict[str, str]) -> bool:
    """Returns True if there is another friendly pawn on the same file."""
    file = square[0]
    pawn_symbol = 'P' if color == 'w' else 'p'
    for rank in '12345678':
        sq = f'{file}{rank}'
        if sq == square:
            continue
        if sq in square_piece_dict and square_piece_dict[sq] == pawn_symbol:
            return True
    return False


def is_isolated_pawn(square: str, color: str, square_piece_dict: Dict[str, str]) -> bool:
    """Returns True if there are no friendly pawns on adjacent files."""
    file_num = LETTER_TO_NUM[square[0]]
    pawn_symbol = 'P' if color == 'w' else 'p'
    adjacent_files = []
    if file_num > 1:
        adjacent_files.append(NUM_TO_LETTER[file_num - 1])
    if file_num < 8:
        adjacent_files.append(NUM_TO_LETTER[file_num + 1])
...
def has_connected_pawn(square: str, color: str, square_piece_dict: Dict[str, str]) -> bool:
    """Returns True if there is a friendly pawn on an adjacent file at the same or +/-1 rank."""
    file_num = LETTER_TO_NUM[square[0]]
    rank = int(square[1])
    pawn_symbol = 'P' if color == 'w' else 'p'
    adjacent_files = []
...
def count_king_shield_pawns(king_sq: str, color: str, square_piece_dict: Dict[str, str]) -> int:
    """Counts friendly pawns on the 2nd/3rd rank directly in front of the king (same file +/- 1)."""
    file_num = LETTER_TO_NUM[king_sq[0]]
    shield_files = []
    for offset in (-1, 0, 1):
        f = file_num + offset
        if 1 <= f <= 8:
            shield_files.append(NUM_TO_LETTER[f])
    pawn_symbol = 'P' if color == 'w' else 'p'
    if color == 'w':
        shield_ranks = ('2', '3')
    else:
    ...
    for f in shield_files:
        for r in shield_ranks:
            ...
            if sq in square_piece_dict and square_piece_dict[sq] == pawn_symbol:
                count += 1
                break  # count at most one pawn per file
    return count


def is_knight_outpost(square: str, color: str, square_piece_dict: Dict[str, str],
                      own_pawn_controlled_squares: List[str]) -> bool:
    """
    A knight is an outpost if:
      1) It sits on rank 4-6 (relative to its color).
      2) It is supported by a friendly pawn.
      3) No enemy pawn can attack it (no enemy pawns on adjacent files at same rank or ahead).
    """
    rank = int(square[1])
    if color == 'w':
        if rank < 4 or rank > 6:
    ...
    if square not in own_pawn_controlled_squares:
        return False

    file_num = LETTER_TO_NUM[square[0]]
    enemy_pawn_symbol = 'p' if color == 'w' else 'P'
    adjacent_files = []
    if file_num > 1:
        adjacent_files.append(NUM_TO_LETTER[file_num - 1])
    if file_num < 8:
        adjacent_files.append(NUM_TO_LETTER[file_num + 1])
    if color == 'w':
        check_ranks = range(rank, 9)
    else:
        check_ranks = range(rank, 0, -1)
    for adj_file in adjacent_files:
        for r in check_ranks:
            sq = f'{adj_file}{r}'
            if sq in square_piece_dict and square_piece_dict[sq] == enemy_pawn_symbol:
                return False
    return True


def count_piece_mobility(position: Position, color: str, piece: str, square: str) -> int:
    """Counts the number of squares a piece can move to (pseudo-legal)."""
    if piece in ('Q', 'R', 'B', 'N', 'K'):
        return len(position.scan_non_pawn_piece_moves(color, piece, square))
    return 0


def has_back_rank_weakness(king_sq: str, color: str, square_piece_dict: Dict[str, str]) -> bool:
    """
    Returns True if the king is on the back rank with no pawn escape square
    (all three squares directly in front are occupied or absent).
    """
    back_rank = '1' if color == 'w' else '8'
    if king_sq[1] != back_rank:
        return False
    escape_rank = '2' if color == 'w' else '7'
    file_num = LETTER_TO_NUM[king_sq[0]]
    for offset in (-1, 0, 1):
        f = file_num + offset
        if 1 <= f <= 8:
            sq = f'{NUM_TO_LETTER[f]}{escape_rank}'
            if sq not in square_piece_dict:
                return False
    return True


def rook_is_behind_passed_pawn(rook_sq: str, pawn_sq: str, color: str) -> bool:
    """Returns True if the rook is behind the passed pawn on the same file."""
    if rook_sq[0] != pawn_sq[0]:
        return False
    rook_rank = int(rook_sq[1])
    pawn_rank = int(pawn_sq[1])
    if color == 'w':
        return rook_rank < pawn_rank
    else:
        return rook_rank > pawn_rank


# ---------------------------------------------------------------------------
# Build pawn control score tables (once, not per call)
# ---------------------------------------------------------------------------

def _build_pawn_control_scores(params: Dict[int, float]):
    ...
    seventh_rank_scores = {f: seventh_rank for f in 'abcdefgh'}
    eighth_rank_scores = {f: eighth_rank for f in 'abcdefgh'}
    sixth_rank_scores = {'a': all_others, 'b': knight_file_6_th_rank, 'c': bishop_file_6_th_rank,
                         'd': central_file_6_th_rank, 'e': central_file_6_th_rank,
                         'f': bishop_file_6_th_rank, 'g': knight_file_6_th_rank, 'h': all_others}
    fifth_rank_scores = {'a': all_others, 'b': knight_file_5_th_rank, 'c': bishop_file_5_th_rank,
                         'd': central_file_5_th_rank, 'e': central_file_5_th_rank,
                         'f': bishop_file_5_th_rank, 'g': knight_file_5_th_rank, 'h': all_others}
    fourth_rank_scores = {'a': all_others, 'b': all_others, 'c': bishop_file_4_th_rank,
                          'd': central_file_4_th_rank, 'e': central_file_4_th_rank,
                          'f': bishop_file_4_th_rank, 'g': all_others, 'h': all_others}
    rank_scores = {'4': fourth_rank_scores, '5': fifth_rank_scores, '6': sixth_rank_scores,
                   '7': seventh_rank_scores, '8': eighth_rank_scores}

    white_pawn_control_scores = {}
    for square in ALL_SQUARES:
        file, rank = square[0], square[1]
        white_pawn_control_scores[square] = rank_scores[rank][file] if rank in rank_scores else all_others

    black_pawn_control_scores = {}
    for square in white_pawn_control_scores:
        rank = int(square[1])
        mirrored_square = square[0] + str(9 - rank)
        black_pawn_control_scores[mirrored_square] = white_pawn_control_scores[square]

    return white_pawn_control_scores, black_pawn_control_scores
...
def quick_evaluate(position: Position, bot_params: Dict[int, float] = None) -> Dict[str, float]:
    """
    Returns an evaluation of the given position from the point of view of the
    side *not* to move.
    ...
    Returns:
        Dict[str, float]: Dictionary with keys 'eval' and 'threat'.
    """
    params = bot_params if bot_params is not None else {}

    # -----------------------------------------------------------------------
    # Parameters 0-39: original bot1 parameters (same defaults)
    # -----------------------------------------------------------------------
    activity_base_score = params.get(11, 0.1)
    central_square_bonus = params.get(12, 0.04)
    sixth_rank_piece_control_bonus = params.get(13, 0.04)
    square_around_enemy_king = params.get(14, 0.03)
    development_score_penalty = -1 * params.get(15, 0.4)
    seventh_rank_bonus = params.get(16, 0.05)
    centralized_knight_bonus = params.get(17, 0.08)
    ...
    unique_square_around_enemy_king_score = params.get(28, 0.08)
    unique_square_around_enemy_king_threat_score = params.get(29, 0.4)
    supported_queen_around_enemy_king_score = params.get(30, 0.08)
    supported_queen_around_enemy_king_threat_score = params.get(31, 0.5)
    ...
    # -----------------------------------------------------------------------
    # Parameters 40-51: new bot2 parameters
    # -----------------------------------------------------------------------
    doubled_pawn_penalty = -1 * params.get(40, 0.25)
    isolated_pawn_penalty = -1 * params.get(41, 0.3)
    connected_pawn_bonus = params.get(42, 0.1)
    king_shield_pawn_score = params.get(43, 0.2)
    missing_king_shield_penalty = -1 * params.get(44, 0.3)
    castling_rights_bonus = params.get(45, 0.2)
    lost_castling_penalty = -1 * params.get(46, 0.15)
    knight_outpost_bonus = params.get(47, 0.25)
    trapped_piece_penalty = -1 * params.get(48, 0.2)
    trapped_piece_mobility_threshold = 2  # a piece with <= this many moves is "trapped"
    rook_behind_passed_pawn_bonus = params.get(49, 0.25)
    fork_threat_score = 2 * params.get(50, 0.5)
    back_rank_weakness_threat_score = 3 * params.get(51, 0.5)

    # -----------------------------------------------------------------------
    # Pawn control score tables
    # -----------------------------------------------------------------------
    white_pawn_control_scores, black_pawn_control_scores = _build_pawn_control_scores(params)
    ...
    # -----------------------------------------------------------------------
    # Checkmate / check detection (unchanged from bot1)
    # -----------------------------------------------------------------------
    ...
    if check_given:
        ...
        checking_pieces = own_square_covering_piece_dict[opposing_king_square]
    ...
    # -----------------------------------------------------------------------
    # Collect pawn-controlled squares for outpost detection
    # -----------------------------------------------------------------------
    own_pawn_controlled_squares_set = []
    for pns in own_piece_covered_square_dict:
        if pns[0] == 'P':
            own_pawn_controlled_squares_set.extend(own_piece_covered_square_dict[pns])
    opposing_pawn_controlled_squares_set = []
    for pns in opposing_piece_covered_square_dict:
        if pns[0] == 'P':
            opposing_pawn_controlled_squares_set.extend(opposing_piece_covered_square_dict[pns])

    # -----------------------------------------------------------------------
    # Piece-square evaluation loop (enhanced with new heuristics)
    # -----------------------------------------------------------------------
    own_passed_pawns = {}
    own_passed_pawn_squares = []
    opposing_passed_pawn_squares = []
    ...
    for sq in square_piece_dict:
        ...
        if piece.upper() == 'R':
            ...
            # NEW: trapped piece detection for rooks
            if not is_endgame:
                mobility = count_piece_mobility(position, color, 'R', sq)
                if mobility <= trapped_piece_mobility_threshold:
                    score += trapped_piece_penalty if own_piece else -trapped_piece_penalty
            ...
            if piece.upper() == 'N':
                ...
                pawn_squares_for_outpost = own_pawn_controlled_squares_set if own_piece else opposing_pawn_controlled_squares_set
                if is_knight_outpost(sq, color, square_piece_dict, pawn_squares_for_outpost):
                    score += knight_outpost_bonus if own_piece else -knight_outpost_bonus
            if piece.upper() == 'B':
            ...
            # NEW: trapped piece detection for minor pieces
            if not is_endgame:
                mobility = count_piece_mobility(position, color, piece.upper(), sq)
                if mobility <= trapped_piece_mobility_threshold:
                    score += trapped_piece_penalty if own_piece else -trapped_piece_penalty

        elif piece.upper() == 'P':
            # NEW: doubled pawn penalty
            if is_doubled_pawn(sq, color, square_piece_dict):
                score += doubled_pawn_penalty / 2 if own_piece else -doubled_pawn_penalty / 2  # halved because each pawn in the pair gets counted

            # NEW: isolated pawn penalty
            if is_isolated_pawn(sq, color, square_piece_dict):
                score += isolated_pawn_penalty if own_piece else -isolated_pawn_penalty

            # NEW: connected pawn bonus
            if has_connected_pawn(sq, color, square_piece_dict):
                score += connected_pawn_bonus if own_piece else -connected_pawn_bonus

            # Passed pawn detection (original)
            n_pawns_in_front = count_pawns_in_front_on_file(sq, color, square_piece_dict)
            if n_pawns_in_front == 0:
                ...
                for tsq in yet_traversed_squares:
                    if tsq in square_covering_piece_dict:
                        if any([pns.startswith('P') for pns in square_covering_piece_dict[tsq]]):
                ...
                if not sq_in_front_attacked:
                    ...
                    if own_piece:
                        ...
                        own_passed_pawn_squares.append(sq)
                        ...
                        opposing_passed_pawn_squares.append(sq)

            # Promotion threats (original)
            if own_piece:
                seventh_rank_val, promotion_rank = (7, 8) if piece == 'P' else (2, 1)
                if int(sq[1]) == seventh_rank_val:
    ...
    # -----------------------------------------------------------------------
    # NEW: Rook behind passed pawn
    # -----------------------------------------------------------------------
    own_rook_squares = position.get_pieces_by_color(side_evaluating_for).get_piece_type_squares('R')
    opposing_rook_squares = position.get_pieces_by_color(side_to_move).get_piece_type_squares('R')

    for rook_sq in own_rook_squares:
        for pawn_sq in own_passed_pawn_squares:
            if rook_is_behind_passed_pawn(rook_sq, pawn_sq, side_evaluating_for):
                score += rook_behind_passed_pawn_bonus
    for rook_sq in opposing_rook_squares:
        for pawn_sq in opposing_passed_pawn_squares:
            if rook_is_behind_passed_pawn(rook_sq, pawn_sq, side_to_move):
                score -= rook_behind_passed_pawn_bonus

    # -----------------------------------------------------------------------
    # NEW: King safety - pawn shield and castling rights (middlegame only)
    # -----------------------------------------------------------------------
    if not is_endgame:
        own_pieces_obj = position.get_pieces_by_color(side_evaluating_for)
        opp_pieces_obj = position.get_pieces_by_color(side_to_move)

        # Pawn shield
        own_shield = count_king_shield_pawns(own_king_square, side_evaluating_for, square_piece_dict)
        opp_shield = count_king_shield_pawns(opposing_king_square, side_to_move, square_piece_dict)
        score += own_shield * king_shield_pawn_score
        score -= opp_shield * king_shield_pawn_score
        if own_shield == 0:
            score += missing_king_shield_penalty
        if opp_shield == 0:
            score -= missing_king_shield_penalty

        # Castling rights
        own_can_castle = own_pieces_obj.can_short_castle() or own_pieces_obj.can_long_castle()
        opp_can_castle = opp_pieces_obj.can_short_castle() or opp_pieces_obj.can_long_castle()
        own_has_castled = (not own_can_castle) and own_king_square[1] in ('1' if side_evaluating_for == 'w' else '8',) and own_king_square[0] in ('g', 'c')
        opp_has_castled = (not opp_can_castle) and opposing_king_square[1] in ('1' if side_to_move == 'w' else '8',) and opposing_king_square[0] in ('g', 'c')

        if own_can_castle and not own_has_castled:
            score += castling_rights_bonus
        elif not own_can_castle and not own_has_castled:
            score += lost_castling_penalty
        if opp_can_castle and not opp_has_castled:
            score -= castling_rights_bonus
        elif not opp_can_castle and not opp_has_castled:
            score -= lost_castling_penalty

    # -----------------------------------------------------------------------
    # NEW: Back rank weakness
    # -----------------------------------------------------------------------
    own_back_rank_weak = has_back_rank_weakness(own_king_square, side_evaluating_for, square_piece_dict)
    opp_back_rank_weak = has_back_rank_weakness(opposing_king_square, side_to_move, square_piece_dict)
    if opp_back_rank_weak:
        # Check if we have a rook or queen aimed at the back rank
        opp_back_rank = '8' if side_to_move == 'w' else '1'
        for pns in own_piece_covered_square_dict:
            if pns[0] in ('R', 'Q'):
                for covered_sq in own_piece_covered_square_dict[pns]:
                    if covered_sq[1] == opp_back_rank:
                        threat_score += back_rank_weakness_threat_score * overwhelming_material_multiplier
                        break
                else:
                    continue
                break
    if own_back_rank_weak:
        own_back_rank_val = '1' if side_evaluating_for == 'w' else '8'
        for pns in opposing_piece_covered_square_dict:
            if pns[0] in ('R', 'Q'):
                for covered_sq in opposing_piece_covered_square_dict[pns]:
                    if covered_sq[1] == own_back_rank_val:
                        score -= back_rank_weakness_threat_score / 2
                        break
                else:
                    continue
                break
    ...
    # -----------------------------------------------------------------------
    # Piece activity / square control (original, both sides)
    # -----------------------------------------------------------------------
    ...
    for pns in own_piece_covered_square_dict:
        ...
        if piece in ('Q', 'R', 'B', 'N'):
            for covered_square in squares_covered:
                ...
                if piece == 'R':
                    seventh_rank_val = '7' if side_evaluating_for == 'w' else '2'
                    if covered_square[1] == seventh_rank_val:
        ...
        if piece in ('Q', 'R', 'B', 'N'):
            for covered_square in squares_covered:
                ...
                if covered_square in own_piece_covered_square_dict[f'K{own_king_square}'] + [own_king_square]:
                    score -= square_around_enemy_king
                if piece == 'R':
                    seventh_rank_val = '7' if side_to_move == 'w' else '2'
                    if covered_square[1] == seventh_rank_val:
                        score -= seventh_rank_bonus
    ...
    # -----------------------------------------------------------------------
    # Hanging material detection (original)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Pressure and threats against opponent (original + fork detection)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # NEW: Fork detection
    # A piece attacks two or more higher-value enemy pieces (or king + piece).
    # -----------------------------------------------------------------------
    for pns in own_piece_covered_square_dict:
        piece_type = pns[0]
        if piece_type == 'K':
            continue
        attacker_value = MATERIAL_DICT[piece_type]
        squares_covered = own_piece_covered_square_dict[pns]
        attacked_valuable_targets = []
        for covered_sq in squares_covered:
            if covered_sq in opposing_squares_occupied:
                target_piece = square_piece_dict[covered_sq].upper()
                target_value = MATERIAL_DICT[target_piece]
                if target_piece == 'K' or target_value > attacker_value:
                    attacked_valuable_targets.append(target_value)
        if len(attacked_valuable_targets) >= 2:
            # Sort descending; the threat is the second-most-valuable target
            # (the opponent can only save one)
            attacked_valuable_targets.sort(reverse=True)
            fork_value = attacked_valuable_targets[1] * material_threat_score_factor
            if pns not in threat_contributing_pieces:
                threat_contributing_pieces[pns] = [fork_threat_score + fork_value]
            else:
                threat_contributing_pieces[pns].append(fork_threat_score + fork_value)

    # -----------------------------------------------------------------------
    # Cleanup: hanging pieces (original)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # King safety: squares around enemy king (original + supported queen)
    # -----------------------------------------------------------------------
